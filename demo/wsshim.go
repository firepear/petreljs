...
func petrelShim(ws *websocket.Conn) {
	...
	pc, err := petrel.TCPClient(&petrel.ClientConfig{Addr: *paddr, Timeout: 2000})
	if err != nil {
	...
	for {
		...
		log.Printf("%d: dispatching request from ws to petrel\n", connnum)
		resp, err := pc.DispatchRaw(msg)
		if err != nil {
			log.Printf("%s\n", resp)
		...
		log.Printf("%d: response received from petrel\n", connnum)
